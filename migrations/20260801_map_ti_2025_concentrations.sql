...
-- Operational mapping derived from the TI-2025 curriculum field descriptions
-- and course titles. It is intentionally independent from the conflicting
-- MK-to-BK tables in the source PDF and is not used as a prerequisite rule.
...
DELETE FROM konsentrasi_mata_kuliah cmk
USING target t
WHERE cmk.konsentrasi_id = t.id;
...
JOIN kurikulum_mata_kuliah kmk
  ON kmk.kurikulum_id = k.id AND kmk.mata_kuliah_id = mk.id
