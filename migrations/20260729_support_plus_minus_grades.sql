...
ALTER TABLE pengambilan_mata_kuliah
  ALTER COLUMN nilai_huruf TYPE VARCHAR(2)
  USING TRIM(nilai_huruf);

ALTER TABLE pengambilan_mata_kuliah
  ADD CONSTRAINT pengambilan_mata_kuliah_nilai_huruf_check CHECK (
    nilai_huruf IS NULL
    OR nilai_huruf IN ('A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D', 'E')
  );

-- Restore grades that were previously normalized when the original extraction is available.
UPDATE pengambilan_mata_kuliah
SET
  nilai_huruf = UPPER(REGEXP_REPLACE(TRIM(raw_result->>'huruf_mutu'), '\s+', '', 'g')),
  bobot_nilai = CASE UPPER(REGEXP_REPLACE(TRIM(raw_result->>'huruf_mutu'), '\s+', '', 'g'))
    WHEN 'A'  THEN 4.00
    WHEN 'A-' THEN 3.75
    WHEN 'B+' THEN 3.50
    WHEN 'B'  THEN 3.00
  ...
  END,
  updated_at = NOW()
WHERE UPPER(REGEXP_REPLACE(TRIM(raw_result->>'huruf_mutu'), '\s+', '', 'g'))
...
SELECT
  ...
  ranked.nilai_huruf IN ('A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D') AS lulus,
...
SELECT
  ...
  COUNT(*) FILTER (WHERE n.nilai_huruf IN ('A', 'A-')) AS jumlah_a,
  COUNT(*) FILTER (WHERE n.nilai_huruf IN ('B+', 'B', 'B-')) AS jumlah_b,
  COUNT(*) FILTER (WHERE n.nilai_huruf IN ('C+', 'C', 'C-')) AS jumlah_c,
...
COMMENT ON VIEW v_nilai_efektif IS
  'Best grade per student/course with plus/minus weights. A-D pass; D needs attention; E must be repeated.';
COMMENT ON VIEW v_ringkasan_akademik IS
  'Academic summary using plus/minus weights, grouped letter counts, the D limit, and mandatory E repeats.';
