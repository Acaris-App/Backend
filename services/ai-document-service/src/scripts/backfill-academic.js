...
const run = async () => {
  const result = await db.query(
    `SELECT id, document_type, isi_teks_dokumen
     FROM dokumen_mahasiswa
     WHERE document_type = 'khs'
       AND NULLIF(BTRIM(isi_teks_dokumen), '') IS NOT NULL
     ORDER BY id`
  );
  ...
  for (const document of result.rows) {
    try {
      const imported = await academicService.importKhs({
        documentId: document.id,
        payload: document.isi_teks_dokumen
      });
      summary.succeeded += 1;
