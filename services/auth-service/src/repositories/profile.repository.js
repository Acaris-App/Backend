...
exports.getMahasiswaProfile = async (userId) => {
  const result = await db.query(`
    SELECT m.angkatan, m.ipk, m.current_semester, m.konsentrasi,
           m.dosen_pa_id,
...
exports.findActiveCurriculumByYearTx = async (client, year) => {
  const result = await client.query(
    `SELECT id FROM kurikulum
     WHERE status = 'aktif' AND tahun_mulai = $1
     LIMIT 1`,
    [year]
  );
  return result.rows[0];
...
exports.hasConcentrationForCurriculumTx = async (client, curriculumId, name) => {
  const result = await client.query(
    `SELECT 1 FROM konsentrasi
     WHERE kurikulum_id = $1 AND nama = $2 AND status = 'aktif'
     LIMIT 1`,
    [curriculumId, name]
  );
