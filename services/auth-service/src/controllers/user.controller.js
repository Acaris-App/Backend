...
exports.updateProfileText = async (req, res, next) => {
  try {
    ...
    if (role === 'mahasiswa' && hasMahasiswaField) {
      ...
      const semester = current_semester !== undefined
        ? parseInt(current_semester)
        : parseInt(currentProfile?.current_semester);
      ...
      let concentrationId;
      if (semester >= 5) {
        const concentration = typeof konsentrasi === 'string'
          ? konsentrasi.trim()
          : currentProfile?.konsentrasi;
        if (!concentration) {
          return res.status(400).json({ status: "error", message: "Konsentrasi wajib diisi untuk semester 5 ke atas" });
        }
        const allowedConcentrations = new Set([
          'Rekayasa Perangkat Lunak', 'Sistem Cerdas', 'Teknik Komputer',
          'Teknologi Informasi', 'Sistem Komputer'
        ]);
        if (!allowedConcentrations.has(concentration)) {
          return res.status(400).json({ status: "error", message: "Konsentrasi tidak valid" });
        }
